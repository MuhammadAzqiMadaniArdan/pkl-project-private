...
<body>
    <div class="wrapper">
        @if (Auth::check())
                ...
                <div class="h-100" id="sb">
                    ...
                    <ul class="sidebar-nav">
                        ...
                        @if (Auth::user()->role == 'admin')
                            <li class="sidebar-item" style="background:black;align-items:baseline;padding-top:10px;">
                        ...
                        @if (Auth::user()->role == 'admin')
                            <li class="sidebar-item">
                                ...
                                <ul id="posts" class="sidebar-dropdown list-unstyled collapse"
                                    data-bs-parent="#sidebar">
                                    {{-- <li class="sidebar-item">
                                        <a href="{{ route('admin.order.data') }}" class="sidebar-link">Data Order</a>
                                    </li>
                                    <li class="sidebar-item">
                                        <a href="{{ route('status.status') }}" class="sidebar-link">Data Status</a>
                                    </li> --}}
                                    <li class="sidebar-item">
            ...
            <div class="main" id="main" role="navigation" aria-label="my-main-navigation">
                ...
                <nav class="navbar navbar-expand px-3 border-bottom" style="box-shadow: 0 1px 2px rgba(0, 0, 0, 5);">
                    <button class="btn" id="sidebar-toggle" type="button" data-toggle="collapse" data-target="#sidebar">
                    ...
                    @if (Auth::user()->role == 'admin')
                        <form action="{{ route('user.search') }}" method="GET" class="w-25"
                            style="margin-left:50px;">

                            <div class="form-inline ">
                                <div class="input-group w-75" data-widget="sidebar-search">
                                    
                                    <select class="liveSearch form-control form-select" id="adminSearch" name="searchUser"
                    ...
                    <div class="navbar-collapse navbar">
                        ...
                        <ul class="navbar-nav">
                            ...
                            @php
                                ...
                                // $get = Order::where('user_id',$orderId);
                                // dd(Auth::user()->id,$get);
                            @endphp
                            @if (Auth::user()->role == 'user')

                                @if (session('cart'))
                                    <a href="{{ route('order.cart') }}" class="btn btn-primary p-3"
                            ...
                            @else
                                <a href="{{ route('order.cart') }}" class="btn text-light btn mt-3 mb-3"
                                    role="button" style="margin-right:30px;background:darkgreen;">
                                    <i class="fa fa-shopping-cart" aria-hidden="true"></i>
                                    Cart Empty
                                </a>
        ...
        <li class="nav-item dropdown">
            ...
            <a href="#" data-bs-toggle="dropdown" class="nav-icon pe-md-0">
                <img src="http://pluspng.com/img-png/user-png-icon-big-image-png-2240.png"
                    class="avatar img-fluid rounded" alt="" style="margin-right: 10px;clear:left;">
                {{-- <i class="fa-regular fa-user pe-2"></i> --}}</img>

            </a>
            <div class="dropdown-menu dropdown-menu-end" style="--bs-dropdown-item-padding-x: 4rem;">

                @if (Auth::user()->role == 'user')
                    <a href="{{ route('userAkun.editAkun', Auth::user()->id) }}" class="dropdown-item">Profile</a>
                    {{-- <a href="#" class="dropdown-item" data-toggle="modal" data-target="#exampleModal">Profile</a> --}}
                @else
        ...
        @if (Auth::user()->role == 'admin')
            <h4 class="roleName" style="margin: 6px 10px;clear:left;text-align:center;align-items:center;">
                Administrator</h4>
        @else
            <h4 class="roleName" style="margin: 6px 10px;clear:left;text-align:center;align-items:center;">User</h4>
        @endif
        </ul>
        {{-- modal --}}
        ...
        <main class="content px-3 py-2" id="main">
            ...
            <div class="container-fluid">
                <div class="container content px-3 py-2">
                    @yield('content')
    ...
    <script src="//maxcdn.bootstrapcdn.com/bootstrap/3.3.1/js/bootstrap.min.js" type="text/javascript" ></script> --}}
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        const a = document.getElementsByClassName("sidebar-link");

                        if (currentTheme) {
                            document.body.classList.add(currentTheme);
                            currentTheme == 'dark-theme';
                            themeSwitch.checked = currentTheme;
                            updateSidebarColor(currentTheme);
                        }
                        ...
                        function updateSidebarColor(theme) {
                            if (theme === 'dark-theme') {
                                sb.style.background = '#333'; // Change this to the desired dark color
                                sb.style.color = '#ecf0f1'; // Change this to the desired dark color
                                main.style.background =
                                    'linear-gradient(to bottom, #d7d8da, gainsboro,rgb(177, 177, 177))'; // Change this to the desired dark color
                                main.style.color = 'black'; // Change this to the desired dark color
                            } else {
                                sb.style.background =
                                    'linear-gradient(to bottom,aqua,white)'; // Change this to the desired light color
                                // sb.style.background = 'linear-gradient(to bottom, #d7d8da, gainsboro,rgb(177, 177, 177))'; // Change this to the desired light color
                                sb.style.color = 'black'; // Change this to the desired light color
                                // item.style.color = 'red'; // Change this to the desired light color
                                main.style.background =
                                    'linear-gradient(to bottom, darkblue,blue,aqua)'; // Change this to the desired light color
                                // main.style.background = 'var(--bs-dark)'; // Change this to the desired light color
                                main.style.color = 'white'; // Change this to the desired light color
                            }
                        }
                    });
                </script>
