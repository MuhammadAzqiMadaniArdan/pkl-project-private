...
@section('content')
    <style>
     ...
     $data1 = [];
     $dataBulan = [];
    foreach ($EntryAll[0] as $key) {
             # code...
             $data2 = $key;
             array_push($data1,$data2);
         
        //  foreach ($key[1] as $bulan) {
        //      $data3 = $bulan;
        //      array_push($dataBulan,$data3);
        //  }
        //  foreach ($key[1] as $bulan) {
        //      $data3 = $bulan;
        //      array_push($dataBulan,$data3);
        ...
        foreach ($EntryAll[1] as $bulan) {
             $data3 = $bulan;
             array_push($dataBulan,$data3);
         }
        //  dd($data1,$lunasChecked);
    ...
    {{-- <form id="cart" action="{{ route('order.addToCart',$userData->id) }}" class="card p-4 mt-5" method="GET"> --}}
        ...
        @if($lunasChecked == false)
            ...
            <form action="{{ route('admin.order.adminStore') }}" class="card p-4 mt-5" method="POST">
                @else
                @if($orderEntry !== 0)
                <form action="{{ route('status.lunasUpdate',$orderEntry['id']) }}" class="card p-4 mt-5" method="POST">
                    @method('PATCH')
                    @else
                    {{-- @dd($validate) --}}
                    <form action="{{ route('admin.order.adminStore') }}" class="card p-4 mt-5" method="POST">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <select id="products" class="form-control form-select" style="width:88%" name="products[]">
                        ...
                        <option disabled hidden selected>Pilih</option>
                            @if ($data1[0]['type'] == 'colocation')
                               
                                    @if ($productsGet['stock'] == 0)
                                        <option value="{{ $data1[0]['id'] }}" data-price="{{ $data1[0]['price'] }}"
                                            data-type="{{ $data1[0]['type'] }}" data-bulan="{{$dataBulan[0]}}" disabled style="background: red;">
                                            <p>(Colocation (HABIS)) {{ $data1[0]['name_product'] }}</p>
                                        </option>
                                    @else
                                        <option value="{{ $data1[0]['id'] }}"
                                            data-price="{{ $data1[0]['price'] }}"
                                            data-type="{{ $data1[0]['type'] }}"  data-bulan="{{$dataBulan[0]}}">
                                            @if ($data1[0]['type'] == 'colocation' && $productsGet['stock'] == 0)
                                                <p disbled style="background: red;">(Colocation (HABIS))
                                                    {{ $data1[0]['name_product'] }}</p>
                                            @else
                                                <p>(Colocation) {{ $data1[0]['name_product'] }}</p>
                                            @endif
                                        </option>
                                    @endif
        ...
        <div class="mb-3 row" style="flex-wrap:nowrap;">
            ...
            <input type="number" name="bandwidth" id="bandwidth" class="form-control"
                style="width:10%;height:10%;margin-right:10px;margin-left:10px;clear:left;" value="{{$data1[1]['qty']}}">
        ...
        {{-- label Server --}}
        <div class="mb-3 row">
            <label for="serverLabel" class="form-label" style="width: 12%">Data Center:</label>
            @if($EntryAll[2]['datacenter'] == "Bogor")
            <input type="text" name="serverLabel" id="serverLabel" class="form-control" value="{{$EntryAll[2]['datacenter']}}" style="width:88%;"> 
            @else 
            <input type="text" name="serverLabel" id="serverLabel" class="form-control" value="{{$EntryAll[2]['datacenter']}}" style="width:88%"> 
            @endif
        </div>
        <div class="mb-3 row">
            ...
            <input type="text" name="serverLabel" id="serverLabel" class="form-control" value="{{$EntryAll[3]['label']}}" style="width:88%">
        </div>
        <br>
        {{-- )))))))))))))))))))))))MEnu Utama ((((((((((((((((( --}}
        ...
        <div class="mb-3 row">
            ...
            <div class="col-sm-10">
                <select id="type" class="form-control" name="type">
                    ...
                    <option value="{{$existingServer['type']}}">
                        {{$existingServer['type']}}
                    </option>
                    @endif
                </select>
        ...
        <div class="mb-3 row">
            ...
            <div class="col-sm-10">
                ...
                @else
                <input type="text" class="form-control" id="series" name="series" value="{{$existingServer['series']}}">
                @endif
            </div>
        </div>
        <div class="mb-3 row">
            ...
            <div class="col-sm-10">
                <select id="dimension" class="form-control" name="dimension">
                    @if($data1[0]['id'] == 6 || $data1[0]['id'] == 10)
                    <option value="1U" selected>
                        1U
                    </option>
                    @elseif($data1[0]['id'] == 9 || $data1[0]['id'] == 11)
                    <option value="2U" selected>
                        2U
                    </option>
                    @else
        ...
        <div class="mb-3 row">
            ...
            <div class="col-sm-10">
                ...
                @else
                <input type="text" class="form-control" id="serialNumber" name="serialNumber" value="{{$existingServer['serialNumber']}}">
                @endif
            </div>
        </div>
