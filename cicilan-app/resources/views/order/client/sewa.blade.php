...
@section('content')
    ...
    <form action="{{ route('status.sewaDate') }}" method="GET">
        ...
        <div class="form-inline">
            ...
            <div class="input-group w-50" data-widget="sidebar-search">
                ...
                <a href="{{ route('status.sewaIndex') }}" class="btn btn-danger ms-2" style="border-radius:5px;">reset</a>
            </div>
        </div>
    </form>
    {{-- @dd(data_get($sewaProducts,'0',true)) --}}
    @php
                ...
                <tbody>
                    ...
                    @foreach ($sewaProducts as $order)
                        ...
                        <tr>
                            ...
                            <td>
                                <ol>
                                    @foreach ($sewaProducts as $innerOrder)
                                        ...
                                        @if ($innerOrder['name_customer'] == $order['name_customer'])
                                            ...
                                            @for ($i = 0; $i < 1; $i++)
                                                <li>{{ $innerOrder['products'][$i]['name_product'] }} <small>Rp.
                                                        {{ number_format($innerOrder['products'][$i]['price'], 0, '.', ',') }}<b>(qty
                                                            : {{ $innerOrder['products'][$i]['qty'] }})</b></small> = Rp.
                                                    {{ number_format($innerOrder['products'][$i]['price_after_qty'], 0, '.', ',') }}
                                                </li>
                                            @endfor
                                        @endif
                                    @endforeach
                        ...
                        <a href="{{ route('status.sewa', $order['id']) }}"
                            ...
                            @if ($orderStatus->payment < 1)
                                <td style="padding:10px 10px;background-color:green;color:#f5f5f5;">
                                    ...
                                    <p class="" style="text-align: center">User Sedang Melakukan Penyewaan Server </p>
                                    <hr>
                            ...
                            @elseif($liveInvoice < $endInvoice && $liveInvoice >= $invoice && $paymentMinus < $order['votes'])
                                <td>
                                    ...
                                    <a class="btn btn-success mb-2" id="bayar" href="#">Invoice Send
                        ...
                        <td style="padding:10px 10px;">
                            ...
                            @else
                                ...
                                <a href="{{ route('detail_server.single', $order['user_id']) }}"
                                    class="btn btn-info mt-2 mb-2">Server Single</a>
                            @endif
                        </td>
...
@else
    <div class="card p-5 mt-5 mb-5">
        ...
        <div class="alert alert-primary mt-2 p-5">
            <h3 style="text-align: center;">Belum ada Client dengan Pesanan Sewa ~</h3>
        </div>
