...
class OrderController extends Controller
{
    ...
    
    {
        ...
        if ($request->ram == "custom") {
            ...
            $ramPrice = 0;
            $ramAssoc = [
                "name_product" => "Custom (" . $request->custom_ram . " GB )",
                "type" => "ram",
                "price" => $ramPrice,
                "qty" => $qty,
        ...
        if ($request->has('name_product')) {
            ...
            if ($productColocation) {
                ...
                if ($request->input('freeze') == 3) {
                    ...
                    $baseCost = $productDedicated->price;
                    ...
                    $newProduct->id = $productDedicated->id;
                    $newProduct->name_product = "(FREEZE) " . $productDedicated->name;
                    $newProduct->price = $productDedicated->price;
                    $newProduct->type = $productDedicated->type;
                    $newProduct->qty = 1;
                ...
                // HIYA-----------------------------HIya--------------
                    ...
                    if ($status2->access < 6  && $status2->access > 0 || $status2->access == 1) {
                        ...
                        // } elseif ($selectedMonths == 3) {
                        //     $totalCost = $baseCost * $selectedMonths + $additionalCost;
                        // } elseif ($selectedMonths == 6) {
                        //     // $totalCost = ($baseCost * $selectedMonths) + ($additionalCost * 5); // Additional cost for 5 months
                        //     $totalCost = ($baseCost * $selectedMonths) + $additionalCost; // Additional cost for 5 months
                        // } elseif ($selectedMonths == 12) {
                        //     $totalCost = ($baseCost * $selectedMonths) - 360000 + $additionalCost; // Deduct 360000 for 12 months
                        // };
                        ...
                        $colloMeh = $totalEnd - $existingTotal;
                        ...
                        $productId = data_get($order['products'], '0.id',0);
                        ...
                        if ($existingProduct['id'] == $productId) {
                            ...
                            if ($existingProduct1) {
                                $priceBulan = Product::find($request->bandwidth);
                            ...
                            if ($existingProduct2) {
                                $priceBulan = Product::find($request->IP);
                            ...
                            $existingTotalPrice = $existingProduct1['price_after_qty'] + $existingProduct2['price_after_qty'] + $existingProduct3['price_after_qty'];

                            $order->total_price += $existingTotalPrice + 10;
                            $order->save();
                        } else {
                            // Add new product if existing product ID doesn't match
                            ...
                            if ($existingProduct['name_product'] == $newProduct['name_product']) {
